...
echo "$($PSQL "SELECT DISTINCT name FROM teams FULL JOIN games winner ON teams.team_id=winner.winner_id FULL JOIN games opponent ON teams.team_id=opponent.opponent_id WHERE (winner.round='Eighth-Final' AND winner.year=2014) OR (opponent.round='Eighth-Final' AND opponent.year=2014) ")"
